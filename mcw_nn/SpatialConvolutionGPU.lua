local SpatialConvolutionGPU, parent = torch.class('nn.SpatialConvolutionGPU', 'nn.Module')

function SpatialConvolutionGPU:__init(nInputPlane, nOutputPlane, kW, kH, dW, dH, padding, partialSum)
   parent.__init(self)
...
function SpatialConvolutionGPU:reset(stdv)
   if stdv then
...
function SpatialConvolutionGPU:updateOutput(input)
   input.nn.SpatialConvolutionGPU_updateOutput(self, input)
...
function SpatialConvolutionGPU:updateGradInput(input, gradOutput)
   input.nn.SpatialConvolutionGPU_updateGradInput(self, input, gradOutput)
   return self.gradInput
end

function SpatialConvolutionGPU:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   input.nn.SpatialConvolutionGPU_accGradParameters(self, input, gradOutput, scale)
...
function SpatialConvolutionGPU:copy(sc)
