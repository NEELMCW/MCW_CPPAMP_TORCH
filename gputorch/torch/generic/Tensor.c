...
{
  THTensor *self = (THTensor *)luaT_checkudata(L, 1, torch_Tensor);
  THStorage *storage;
  long storageOffset;
  THLongStorage *size, *stride;
...
static int torch_Tensor_(indexSelect)(lua_State *L)
{
  ...
  if (narg == 3)
  {
    ...
    src = (THTensor *)luaT_checkudata(L, 1, torch_Tensor);
    dim = luaL_checkint(L, 2) - 1;
    index = (THLongTensor *)luaT_checkudata(L, 3, "torch.LongTensor");
    luaT_pushudata(L,tensor,torch_Tensor);
  }
  else if (narg == 4)
  {
    src = (THTensor *)luaT_checkudata(L, 2, torch_Tensor);
    dim = luaL_checkint(L, 3) - 1;
    index = (THLongTensor *)luaT_checkudata(L, 4, "torch.LongTensor");
    tensor = (THTensor *)luaT_checkudata(L,1,torch_Tensor);
  }
  else
  {
...
static int torch_Tensor_(indexCopy)(lua_State *L)
{
  ...
  if (narg == 4)
  {
    dim = luaL_checkint(L, 2) - 1;
    index = (THLongTensor *)luaT_checkudata(L, 3, "torch.LongTensor");
    src = (THTensor *)luaT_checkudata(L, 4, torch_Tensor);
    tensor = (THTensor *)luaT_checkudata(L,1,torch_Tensor);
  }
  else
  {
...
static int torch_Tensor_(__newindex__)(lua_State *L)
{
  ...
  else if ((idx = (THLongStorage *)luaT_toudata(L, 2, "torch.LongStorage")))
  {
  ...
  else if ((mask = (THByteTensor *)luaT_toudata(L, 2, "torch.ByteTensor")))
  {
    THTensor *vals;
    if (lua_isnumber(L, 3))
    {
    ...
    else if ((vals = (THTensor *)luaT_toudata(L, 3, torch_Tensor)))
    {
...
static void torch_Tensor_(c_readSizeStride)(lua_State *L, int index, int allowStride, THLongStorage **size_, THLongStorage **stride_)
{
  ...
  if ( (size = (THLongStorage *)luaT_toudata(L, index, "torch.LongStorage")) )
  {
    if (!lua_isnoneornil(L, index + 1))
    {
      if ( (stride = (THLongStorage *)luaT_toudata(L, index+1, "torch.LongStorage")) )
                                                         ...
                                                         THStorage **storage_, long *storageOffset_, THLongStorage **size_, THLongStorage **stride_)
{
  ...
  else if ( allowTensor && (arg1Type == LUA_TUSERDATA) && (src =(THTensor *)luaT_toudata(L, index, torch_Tensor)) )
  {
  ...
  else if ( allowStorage && (arg1Type == LUA_TUSERDATA) && (storage = (THStorage *)luaT_toudata(L, index, torch_Storage)) )
  {
    *storage_ = storage;
...
{
  THTensor *tensor = (THTensor *)luaT_checkudata(L, 1, torch_Tensor);
  THTensor *src1 = (THTensor *)luaT_checkudata(L, 2, torch_Tensor);
  THTensor *src2 = (THTensor *)luaT_checkudata(L, 3, torch_Tensor);
...
static int torch_Tensor_(read)(lua_State *L)
{
  ...
  tensor->size =(long *) THAlloc(sizeof(long)*tensor->nDimension);
  tensor->stride =(long *) THAlloc(sizeof(long)*tensor->nDimension);
  ...
  tensor->storage = (THStorage *)luaT_toudata(L, -1, torch_Storage);
  if (tensor->storage)
    THStorage_(retain)(tensor->storage);
