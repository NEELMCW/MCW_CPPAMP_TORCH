...
// FIXME: is it correct?
int gpuMemcpyAsync(void* dst, size_t src_offset, void* src,
                           size_t dst_offset, size_t count, gpuMemcpyKind kind)

{
  switch(kind){
    ...
    case gpuMemcpyHostToDevice: {
      cl_event event;
      clEnqueueReadBuffer(Concurrency::getAllocator().getQueue(),
    ...
    case gpuMemcpyDeviceToHost: {
      cl_event event;
      clEnqueueWriteBuffer(Concurrency::getAllocator().getQueue(),
    ...
    case gpuMemcpyDeviceToDevice: {
      cl_event event;
      clEnqueueCopyBuffer (Concurrency::getAllocator().getQueue(),
                           static_cast<cl_mem>(src), static_cast<cl_mem>(dst),
                           src_offset, dst_offset, count, 0, NULL, &event);
      cl_int status = CL_QUEUED;
      while(status != CL_COMPLETE) {
