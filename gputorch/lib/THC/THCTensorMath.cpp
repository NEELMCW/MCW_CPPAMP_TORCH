...
void THCudaTensor_kernel_addcmul(float *data, float value, float *src1, float *src2, long size,
                                const int nThreadPerBlock, int nBlockPerRow, int nBlockPerColumn)
{
...
void THCudaTensor_addcmul(THCudaTensor *self_, THCudaTensor* t, float value, THCudaTensor *src1,
                         THCudaTensor *src2)
{
  if (self_ != t)
  {
...
void THCudaTensor_addcdiv(THCudaTensor *self_, THCudaTensor *t, float value, THCudaTensor *src1,
                         THCudaTensor *src2)
{
  if (self_ != t)
  {
                                                ...
                                                unsigned int gridConf[])
{
  ...
  {
    for (unsigned z = tidx.tile[0]; z < avSize[2] ; z += t_ext[0])
    {
      for (unsigned y = tidx.tile[1]; y < avSize[1] ; y += t_ext[1])  
      {
                                         ...
                                         UnaryFunction unary_op, float init, BinaryFunction binary_op)
{
  ...
  THCudaTensor_kernel_transformReduceOuterDim(THCudaTensor_data(tgt),
                                             THCudaTensor_data(src), THCudaTensor_nElement(src), THCudaTensor_nElement(tgt),
...
void THCudaTensor_transformReduceDim(THCudaTensor *self_, THCudaTensor *src,
                                    long dimension, UnaryFunction unary_op, float init,
...
void THCudaTensor_reduceDim(THCudaTensor *self_, THCudaTensor *src, long dimension, float init,
                           BinaryFunction binary_op)
{
...
void THCudaTensor_clamp(THCudaTensor *self_, THCudaTensor *src, float min_value, float max_value)
{
...
void THCudaTensor_mean(THCudaTensor *self, THCudaTensor *src, long dim)
{
...
struct partial_less_functor
{
  ...
  float operator()(const float &lhs) const
  {
    return lhs < rhs;
  }
};
...
struct partial_greater_functor
{
  ...
  bool operator()(const float &lhs) const
  {
    return lhs > rhs;
  }
};
...
void THCudaTensor_kernel_renorm(float *data, const float value, const long size, const float maxnorm,
                               long gridSz)
{
...
void THCudaTensor_renorm(THCudaTensor* self, THCudaTensor* src, float value, long dimension,
                        float maxnorm)
{
  THCudaTensor *self_;
...
void THCudaTensor_kernel_indexFill(float *tensor, Concurrency::array<long>* stride, float *index,
                                  long src_nDim, int dim, long idx_size, long tensor_size,
                                  long size_dim, float val, long nblockx)
{
...
void THCudaTensor_kernel_indexCopy(float *res, float *src, Concurrency::array<long, 1>* res_stride, float *index,
                                  long res_size, long res_nDim, int dim, long idx_size, long src_size,
...
void THCudaTensor_indexCopy(THCudaTensor *res_, int dim, THLongTensor *indices, THCudaTensor *src)
{
  ...
  THCudaTensor_kernel_indexCopy(THCudaTensor_data(res_), THCudaTensor_data(src), stride_,
                               THCudaTensor_data(indices_), nRes, res_->nDimension, dim, nIndex,
...
void THCudaTensor_indexFill(THCudaTensor *res_, int dim, THLongTensor *indices, float val)
{
  ...
  /*THCudaTensor_kernel_indexFill(THCudaTensor_data(res_), stride_, THCudaTensor_data(indices_),
...
void THCudaTensor_kernel_indexSelect(float *tensor, float *src, Concurrency::array<long>* src_stride,
                                    float *index, long src_nDim, int dim, long idx_size,
                                    long tensor_size, long src_size, long size_dim, long nblockx)
{
...
void THCudaTensor_indexSelect(THCudaTensor *res_, THCudaTensor *src, int dim, THLongTensor *indices)
{
  ...
  THCudaTensor_kernel_indexSelect(THCudaTensor_data(res_), THCudaTensor_data(src), stride_,
                                 THCudaTensor_data(indices_), src->nDimension, dim, indices->size[0],
                                 nRes, THCudaTensor_nElement(src), src->size[dim], nblockx);
