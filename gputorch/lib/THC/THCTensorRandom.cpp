...
void THCRandom_init(THCudaRNGState* state, int devices, int current_device)
{
...
void THCRandom_shutdown(THCudaRNGState* state)
{
...
void THCRandom_setGenerator(THCudaRNGState* state, int device)
{
...
void THCRandom_resetGenerator(THCudaRNGState* state)
{
  initializeGenerator(state->current_gen);
...
/* Random seed */
unsigned long THCRandom_seed(THCudaRNGState* state)
{
...
unsigned long THCRandom_seedAll(THCudaRNGState* state)
{
  unsigned long s = (unsigned long)time(0);
...
void THCRandom_manualSeed(THCudaRNGState* state, unsigned long seed)
{
...
void THCRandom_manualSeedAll(THCudaRNGState* state, unsigned long seed)
{
  int currentDevice;
...
unsigned long THCRandom_initialSeed(THCudaRNGState* state)
{
  return state->current_gen->initial_seed;
}

void THCRandom_getRNGState(THCudaRNGState* state, THByteTensor *rng_state)
{
...
void THCRandom_setRNGState(THCudaRNGState* state, THByteTensor *rng_state)
{
  /*static const size_t states_size = MAX_NUM_BLOCKS * sizeof(curandStateMtgp32);
  static const size_t seed_size = sizeof(unsigned long);
  static const size_t total_size = states_size + seed_size;
