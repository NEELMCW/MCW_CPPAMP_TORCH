...
void img_acts_color(THGPUTensor* hidActsTensor, THGPUTensor* filterTensor, THGPUTensor* targetTensor,
                    ...
                    const int blockX, const int blockY, const int bx)
{
    std::cout<<"img_acts_color"<<std::endl;
            ...
            for (int my = startY; my < endY; my++)
            {
                ...
                for (int mx = startX; mx < endX; mx++)
                {
                    ...
                    {   // multiply with 16 filters at a time
                        ...
                        const int hLoad = (hidActs + (moduleIdx + f * numModules) * numImages);
                        ...
                        for (int i = 0; i < imgsPerThread * 16; i += 32)
                        {
                            if (!checkCaseBounds || blockCaseIdx + i + loadX < numImages)
                            {
                                ...
                                {   // load 16 rows of imgsPerThread*16 cols, 8 * 32 elements at a time.
                                    shHidActLoad[j * 16 * imgsPerThread + i] = avhidActs[hLoad + j * numModules * numImages + i];
                                }
                            }
                            else
                            {
                        ...
                        if (isPxInImg && isPxInModule)
                        {
                            ...
                            const int fLoad = conv ? (filters + pxIdxInModule * numFilters + f)
                                                   : (filters + (moduleIdx * numColors * filterPixels + pxIdxInModule) * numFilters + f);

                            for (int c = 0; c < numColors; c++)
                            {
                                shilterLoad[c * 16 * (16 + 1)] = avFilters[fLoad + c * filterPixels * numFilters];
                            }
                        }
...
void img_acts_mediumcolor(THGPUTensor* hidActsTensor, THGPUTensor* filterTensor, THGPUTensor* targetTensor,
                          ...
                          const int blockX, const int blockY, const int bx)
{
    std::cout<<"img_acts_mediumcolor"<<std::endl;
                ...
                for (int mx = startX; mx < endX; mx++)
                {
                    ...
                    for (int f = 0; f < numFiltersPerGroup; f += 16)
                    {
                        ...
                        if (isPxInImg && isPxInModule) {
                            ...
                            for (int c = 0; c < colorsPerThread; c++)
                            {
                                shFilterLoad[c * 16 * (16 + 1)] = avFilters[ fLoad + c * filterPixels * numFilters];
                            }
                        }
...
void conv_img_acts_manycolor(THGPUTensor* hidActsTensor, THGPUTensor* filterTensor, THGPUTensor* targetTensor,
                             ...
                             const int blockX, const int blockY, const int bx)
{
    std::cout<<"conv_img_acts_manycolor"<<std::endl;
    const int numFilterColors = numImgColors / numGroups;
                    ...
                    for (int f = 0; f < numFiltersPerGroup; f += 16)
                    {
                        ...
                        for (int i = 0; i < imgsPerThread * B_X; i += 32)
                        {
                            if (!checkCaseBounds || blockCaseIdx + hidActLoadX + i < numImages)
                            {
                                ...
                                {
                                    // load 16 rows of imgsPerThread*16 cols, 8 * 32 elements at a time.
                                    shHidActLoad[j * B_X * imgsPerThread + i] = avhidActs[hLoad + j * numModules * numImages + i];
                                }
                            }
                            else
                            {
                        ...
                        const int fLoad = conv ? (filters + pxIdxInFilter * numFilters + f)
                        ...
                        for (int i = 0; i < colorsPerThread*B_Y; i+= B_X*B_Y/16)
                        {
                            ...
                            {
                                shFilterLoad[i * (16 + 1)] = avFilters[fLoad + i * filterPixels * numFilters];
                            }
                        }
                        //__syncthreads();
                        tidx.barrier.wait();
...
void spatialConv_updateGradInput(
    // raw pointers:
    THGPUTensor *hidActs, THGPUTensor *filters, THGPUTensor *targets,
    // input dim:
                    ...
                    if (checkCaseBounds) {
                        if (numFilterColors == 1) {
                            ...
                            img_acts_color<8, 1, false, true, true>(hidActs, filters, targets,
                                                                numModulesY, numModulesX, numImages, numFilters, filterSize, imgSizeY, imgSizeX, paddingStart, moduleStride, scaleTargets, scaleOutput, blockX, blockY, threadX);
